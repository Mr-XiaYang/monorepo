...
function _nullishCoalesce$e(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain$l(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }
const defaultTagPrefix = 'tag:yaml.org,2002:';
function findTagObject(value, tagName, tags) {
    if (tagName) {
        const match = tags.filter(t => t.tag === tagName);
        const tagObj = _nullishCoalesce$e(match.find(t => !t.format), () => ( match[0]));
        if (!tagObj)
...
function getTagObject(tags, item) {
    if (item.tag) {
        ...
        if (match.length > 0)
            return _nullishCoalesce$d(match.find(t => t.format === item.format), () => ( match[0]));
    }
    let tagObj = undefined;
    let obj;
    if (isScalar(item)) {
        obj = item.value;
        const match = tags.filter(t => _optionalChain$j([t, 'access', _ => _.identify, 'optionalCall', _2 => _2(obj)]));
        tagObj =
            _nullishCoalesce$d(match.find(t => t.format === item.format), () => ( match.find(t => !t.format)));
    }
    else {
        obj = item;
    ...
    if (!tagObj) {
        const name = _nullishCoalesce$d(_optionalChain$j([obj, 'optionalAccess', _3 => _3.constructor, 'optionalAccess', _4 => _4.name]), () => ( typeof obj));
...
function stringify(item, ctx, onComment, onChompKeep) {
    ...
    if (props.length > 0)
        ctx.indentAtStart = (_nullishCoalesce$d(ctx.indentAtStart, () => ( 0))) + props.length + 1;
...
function addPairToJSMap(ctx, map, { key, value }) {
    if (_optionalChain$i([ctx, 'optionalAccess', _ => _.doc, 'access', _2 => _2.schema, 'access', _3 => _3.merge]) && isMergeKey(key)) {
...
function _nullishCoalesce$c(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }
function stringifyCollection(collection, ctx, options) {
    const flow = _nullishCoalesce$c(ctx.inFlow, () => ( collection.flow));
...
class YAMLMap extends Collection {
    ...
    add(pair, overwrite) {
        ...
        if (isPair(pair))
            ...
            _pair = new Pair(pair, _optionalChain$g([pair, 'optionalAccess', _2 => _2.value]));
        }
        else
            _pair = new Pair(pair.key, pair.value);
        const prev = findPair(this.items, _pair.key);
        const sortEntries = _optionalChain$g([this, 'access', _3 => _3.schema, 'optionalAccess', _4 => _4.sortMapEntries]);
        if (prev) {
            if (!overwrite)
    ...
    get(key, keepScalar) {
        const it = findPair(this.items, key);
        const node = _optionalChain$g([it, 'optionalAccess', _5 => _5.value]);
        return _nullishCoalesce$b((!keepScalar && isScalar(node) ? node.value : node), () => ( undefined));
    }
    has(key) {
     ...
     */
    toJSON(_, ctx, Type) {
        const map = Type ? new Type() : _optionalChain$g([ctx, 'optionalAccess', _6 => _6.mapAsMap]) ? new Map() : {};
        if (_optionalChain$g([ctx, 'optionalAccess', _7 => _7.onCreate]))
            ctx.onCreate(map);
...
class YAMLSeq extends Collection {
    ...
    toJSON(_, ctx) {
        const seq = [];
        if (_optionalChain$f([ctx, 'optionalAccess', _2 => _2.onCreate]))
            ctx.onCreate(seq);
        let i = 0;
        for (const item of this.items)
...
function _nullishCoalesce$a(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }
            ...
            if (isPair(item))
                ...
                if (item.commentBefore)
                    pair.key.commentBefore = pair.key.commentBefore
                        ...
                        : item.commentBefore;
                if (item.comment) {
                    const cn = _nullishCoalesce$a(pair.value, () => ( pair.key));
...
class Schema {
    constructor({ compat, customTags, merge, resolveKnownTags, schema, sortMapEntries, toStringDefaults }) {
        ...
        this.toStringOptions = _nullishCoalesce$9(toStringDefaults, () => ( null));
...
function stringifyDocument(doc, options) {
    ...
    if (_optionalChain$d([doc, 'access', _ => _.directives, 'optionalAccess', _2 => _2.docEnd])) {
        if (doc.comment) {
...
class Document {
    constructor(value, replacer, options) {
        ...
        this.options = opt;
        let { version } = opt;
        if (_optionalChain$c([options, 'optionalAccess', _ => _._directives])) {
    ...
    createNode(value, replacer, options) {
        ...
        const { aliasDuplicateObjects, anchorPrefix, flow, keepUndefined, onTagObj, tag } = _nullishCoalesce$8(options, () => ( {}));
        const { onAnchor, setAnchors, sourceObjects } = createNodeAnchors(this, 
        // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
        anchorPrefix || 'a');
        const ctx = {
            aliasDuplicateObjects: _nullishCoalesce$8(aliasDuplicateObjects, () => ( true)),
            keepUndefined: _nullishCoalesce$8(keepUndefined, () => ( false)),
            onAnchor,
            onTagObj,
            replacer: _replacer,
            schema: this.schema,
    ...
    toJS({ json, jsonArg, mapAsMap, maxAliasCount, onAnchor, reviver } = {}) {
        ...
        const res = toJS(this.contents, _nullishCoalesce$8(jsonArg, () => ( '')), ctx);
...
function _nullishCoalesce$7(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }function resolveProps(tokens, { flow, indicator, next, offset, onError, startOnNewline }) {
    ...
    for (const token of tokens) {
        ...
        switch (token.type) {
            ...
            case indicator:
                ...
                if (found)
                    onError(token, 'UNEXPECTED_TOKEN', `Unexpected ${token.source} in ${_nullishCoalesce$7(flow, () => ( 'collection'))}`);
                found = token;
                atNewline = false;
                hasSpace = false;
                break;
    ...
    return {
        ...
        start: _nullishCoalesce$7(start, () => ( end))
    };
}
...
function flowIndentCheck(indent, fc, onError) {
    if (_optionalChain$b([fc, 'optionalAccess', _ => _.type]) === 'flow-collection') {
        const end = fc.end[0];
...
function resolveBlockMap({ composeNode, composeEmptyNode }, ctx, bm, onError) {
    ...
    for (const collItem of bm.items) {
        ...
        const keyProps = resolveProps(start, {
            ...
            next: _nullishCoalesce$6(key, () => ( _optionalChain$a([sep, 'optionalAccess', _ => _[0]]))),
            offset,
            onError,
            startOnNewline: true
        });
        ...
        if (implicitKey) {
            ...
            if (keyProps.hasNewlineAfterProp || containsNewline(key)) {
                onError(_nullishCoalesce$6(key, () => ( start[start.length - 1])), 'MULTILINE_IMPLICIT_KEY', 'Implicit keys need to be on a single line');
            }
        }
        else if (_optionalChain$a([keyProps, 'access', _2 => _2.found, 'optionalAccess', _3 => _3.indent]) !== bm.indent) {
            onError(offset, 'BAD_INDENT', startColMsg);
        }
        // key value
        ...
        // value properties
        const valueProps = resolveProps(_nullishCoalesce$6(sep, () => ( [])), {
        ...
        if (valueProps.found) {
            if (implicitKey) {
                if (_optionalChain$a([value, 'optionalAccess', _4 => _4.type]) === 'block-map' && !valueProps.hasNewline)
...
function resolveFlowCollection({ composeNode, composeEmptyNode }, ctx, fc, onError) {
    ...
    for (let i = 0; i < fc.items.length; ++i) {
        ...
        const props = resolveProps(start, {
            flow: fcName,
            indicator: 'explicit-key-ind',
            next: _nullishCoalesce$5(key, () => ( _optionalChain$9([sep, 'optionalAccess', _ => _[0]]))),
            offset,
            onError,
        ...
        if (i === 0) {
            ...
            if (props.comment) {
                ...
                if (prevItemComment) {
                    ...
                    if (isPair(prev))
                        prev = _nullishCoalesce$5(prev.value, () => ( prev.key));
                    if (prev.comment)
                        prev.comment += '\n' + prevItemComment;
                    else
        ...
        if (!isMap && !sep && !props.found) {
            ...
            // value properties
            const valueProps = resolveProps(_nullishCoalesce$5(sep, () => ( [])), {
                flow: fcName,
                indicator: 'map-value-ind',
                next: value,
                offset: keyNode.range[2],
...
function composeCollection(CN, ctx, token, tagToken, onError) {
    ...
    node.tag = tagName;
    if (_optionalChain$8([tag, 'optionalAccess', _ => _.format]))
...
function splitLines(source) {
    ...
    const first = split[0];
    const m = first.match(/^( *)/);
    const line0 = _optionalChain$7([m, 'optionalAccess', _ => _[1]])
...
function foldLines(source) {
    ...
    return res + sep + (_nullishCoalesce$4(_optionalChain$6([match, 'optionalAccess', _2 => _2[1]]), () => ( '')));
}
...
function composeScalar(ctx, token, tagToken, onError) {
    ...
    let scalar;
    try {
        const res = tag.resolve(value, msg => onError(_nullishCoalesce$3(tagToken, () => ( token)), 'TAG_RESOLVE_FAILED', msg), ctx.options);
        scalar = isScalar(res) ? res : new Scalar(res);
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        onError(_nullishCoalesce$3(tagToken, () => ( token)), 'TAG_RESOLVE_FAILED', msg);
...
function findScalarTagByName(schema, value, tagName, tagToken, onError) {
    ...
    for (const tag of matchWithTest)
        if (_optionalChain$5([tag, 'access', _ => _.test, 'optionalAccess', _2 => _2.test, 'call', _3 => _3(value)]))
            return tag;
...
function findScalarTagByTest({ directives, schema }, value, token, onError) {
    const tag = schema.tags.find(tag => tag.default && _optionalChain$5([tag, 'access', _4 => _4.test, 'optionalAccess', _5 => _5.test, 'call', _6 => _6(value)])) || schema[SCALAR$1];
    if (schema.compat) {
        const compat = _nullishCoalesce$3(schema.compat.find(tag => tag.default && _optionalChain$5([tag, 'access', _7 => _7.test, 'optionalAccess', _8 => _8.test, 'call', _9 => _9(value)])), () => (
            schema[SCALAR$1]));
...
function _optionalChain$4(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }function emptyScalarPosition(offset, before, pos) {
    if (before) {
        if (pos === null)
            pos = before.length;
        for (let i = pos - 1; i >= 0; --i) {
            ...
            st = before[++i];
            while (_optionalChain$4([st, 'optionalAccess', _ => _.type]) === 'space') {
                offset += st.source.length;
                st = before[++i];
            }
            break;
...
function composeDoc(options, directives, { offset, start, value, end }, onError) {
    ...
    const props = resolveProps(start, {
        indicator: 'doc-start',
        next: _nullishCoalesce$2(value, () => ( _optionalChain$3([end, 'optionalAccess', _ => _[0]]))),
        offset,
        onError,
        startOnNewline: true
    });
    ...
    for (let i = 0; i < prelude.length; ++i) {
        ...
        switch (source[0]) {
            case '#':
                ...
                break;
            case '%':
                if (_optionalChain$2([prelude, 'access', _ => _[i + 1], 'optionalAccess', _2 => _2[0]]) !== '#')
                    i += 1;
                atComment = false;
                break;
            default:
...
class Lexer {
    ...
    *lex(source, incomplete = false) {
        ...
        let next = _nullishCoalesce$1(this.next, () => ( 'stream'));
...
function isFlowToken(token) {
    switch (_optionalChain$1([token, 'optionalAccess', _ => _.type])) {
        case 'alias':
        case 'scalar':
        case 'single-quoted-scalar':
        case 'double-quoted-scalar':
...
function getPrevProps(parent) {
    switch (parent.type) {
        ...
        case 'block-map': {
            ...
            return _nullishCoalesce(it.sep, () => ( it.start));
        }
        case 'block-seq':
...
function getFirstKeyStartProps(prev) {
    ...
    while (_optionalChain$1([prev, 'access', _2 => _2[++i], 'optionalAccess', _3 => _3.type]) === 'space') {
        /* loop */
    }
...
class Parser {
    ...
    *pop(error) {
        const token = _nullishCoalesce(error, () => ( this.stack.pop()));
        /* istanbul ignore if should not happen */
        if (!token) {
        ...
        switch (this.type) {
            case 'newline':
                ...
                if (it.value) {
                    ...
                    if (_optionalChain$1([last, 'optionalAccess', _4 => _4.type]) === 'comment')
                        _optionalChain$1([end, 'optionalAccess', _5 => _5.push, 'call', _6 => _6(this.sourceToken)]);
                    else
            ...
            case 'comment':
                if (it.value) {
                    ...
                    if (this.atIndentedComment(it.start, map.indent)) {
                        ...
                        const end = _optionalChain$1([prev, 'optionalAccess', _7 => _7.value, 'optionalAccess', _8 => _8.end]);
        ...
        switch (this.type) {
            case 'newline':
                if (it.value) {
                    const end = 'end' in it.value ? it.value.end : undefined;
                    const last = Array.isArray(end) ? end[end.length - 1] : undefined;
                    if (_optionalChain$1([last, 'optionalAccess', _9 => _9.type]) === 'comment')
                        _optionalChain$1([end, 'optionalAccess', _10 => _10.push, 'call', _11 => _11(this.sourceToken)]);
                    else
            ...
            case 'comment':
                if (it.value)
                    ...
                    if (this.atIndentedComment(it.start, seq.indent)) {
                        ...
                        const end = _optionalChain$1([prev, 'optionalAccess', _12 => _12.value, 'optionalAccess', _13 => _13.end]);
                        if (Array.isArray(end)) {
                            Array.prototype.push.apply(end, it.start);
                            end.push(this.sourceToken);
                            seq.items.pop();
...
function parseDocument(source, options = {}) {
    ...
    const parser = new Parser(_optionalChain([lineCounter, 'optionalAccess', _2 => _2.addNewLine]));
...
var config = fastifyPlugin__default["default"](async (fastify, opts) => {
  ...
  if (configPath) {
    if (fs__default["default"].existsSync(configPath)) {
      config = Object.assign(defaultConfig, parseDocument(fs__default["default"].readFileSync(configPath, "utf-8")).toJS());
    } else {
      fs__default["default"].mkdirSync(path__default["default"].dirname(configPath), {recursive: true});
      fs__default["default"].writeFileSync(configPath, new Document(defaultConfig).toString());
    }
  }
